"""
Improved Database Migration Script (v2)
...
def split_sql_statements(sql_content):
    """
    Smart SQL splitter that handles PostgreSQL functions with $$ delimiters
    """
    statements = []
    current_statement = []
    in_function = False
...
def run_migration_with_psycopg2():
    ...
    try:
            ...
            with open(migration_file, 'r', encoding='utf-8') as f:
                ...
                for i, statement in enumerate(statements, 1):
                    try:
                        ...
                        error_msg = str(e)
                        # Skip if table/index/trigger already exists
                        if any(x in error_msg.lower() for x in ['already exists', 'duplicate']):
        ...
        cursor.execute("""
            ...
            AND (table_name LIKE 'offer%' OR table_name LIKE 'supplier%' OR table_name LIKE 'purchase%' OR table_name = 'marketplaces')
            ORDER BY table_name;
        """)
        ...
        print("3. Try manual migration (see DOCKER_DATABASE_MIGRATION_GUIDE.md)")
        return False
...
def main():
    print("="*60)
    print("DATABASE MIGRATION SCRIPT (v2)")
