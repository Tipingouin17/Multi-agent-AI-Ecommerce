-- ============================================================================
-- OFFERS MANAGEMENT SCHEMA (FIXED VERSION)
-- ============================================================================
-- This migration adds tables for managing special offers, promotions, and deals
-- Fixed to remove dependency on marketplaces table
-- ============================================================================
...
-- Create indexes for offer_usage
CREATE INDEX IF NOT EXISTS idx_offer_usage_offer ON offer_usage(offer_id);
CREATE INDEX IF NOT EXISTS idx_offer_usage_customer ON offer_usage(customer_id);
CREATE INDEX IF NOT EXISTS idx_offer_usage_order ON offer_usage(order_id);
...
CREATE INDEX IF NOT EXISTS idx_offer_marketplaces_marketplace ON offer_marketplaces(marketplace_id);

-- Update timestamp trigger function (create if not exists)
...
DROP TRIGGER IF EXISTS offer_analytics_updated_at_trigger ON offer_analytics;
CREATE TRIGGER offer_analytics_updated_at_trigger
    BEFORE UPDATE ON offer_analytics
    FOR EACH ROW
    EXECUTE FUNCTION update_offers_updated_at();

DROP TRIGGER IF EXISTS marketplaces_updated_at_trigger ON marketplaces;
CREATE TRIGGER marketplaces_updated_at_trigger
...
COMMENT ON TABLE marketplaces IS 'E-commerce marketplace integrations';
