...
// ============================================
// NEW: Conditional Layout Wrapper Component
// This component handles interface selection INSIDE the route tree
// ============================================
function ConditionalLayout({ selectedInterface, onInterfaceReset, children }) {
  const location = useLocation();
  const navigate = useNavigate();

  // If no interface selected and not on login/register, show selector
  if (!selectedInterface && location.pathname !== '/login' && location.pathname !== '/register') {
    return <Navigate to="/interface-selector" replace />;
  }

  // If interface selected but on login/register, redirect to appropriate dashboard
  if (selectedInterface && (location.pathname === '/login' || location.pathname === '/register')) {
    if (selectedInterface === 'admin') return <Navigate to="/admin/dashboard" replace />;
    if (selectedInterface === 'merchant') return <Navigate to="/merchant/dashboard" replace />;
    if (selectedInterface === 'customer') return <Navigate to="/customer" replace />;
    if (selectedInterface === 'database-test') return <Navigate to="/database-test" replace />;
  }
...
function App() {
  ...
  // Use ref to prevent state updates during navigation
  ...
  const checkDatabaseHealth = async () => {
    try {
      ...
      if ('requestIdleCallback' in window) {
        requestIdleCallback(() => performHealthCheck());
      } else {
  ...
  const performHealthCheck = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setDatabaseStatus(prev => prev.connected ? { connected: false, healthyAgents: 0, totalAgents: 0 } : prev);
      }
    } catch (error) {
      setDatabaseStatus(prev => prev.connected ? { connected: false, healthyAgents: 0, totalAgents: 0 } : prev);
    }
      ...
      <WebSocketProvider>
        <BrowserRouter>
          {/* ============================================
              ...
              No conditional route definitions
              ============================================ */}
          <Routes>
            {/* ============================================
            ...
            {/* ============================================
                INTERFACE SELECTOR (Always accessible)
                ============================================ */}
            <Route 
            ...
            {/* ============================================
                ...
                Redirects based on selectedInterface
                ============================================ */}
            <Route 
              path="/" 
              element={
                selectedInterface ? (
                  ...
                  selectedInterface === 'customer' ? <Navigate to="/customer" replace /> :
                  selectedInterface === 'database-test' ? <Navigate to="/database-test" replace /> :
            ...
            <Route 
              ...
              element={
                <ProtectedRoute allowedRoles={['admin']}>
                  <AdminLayout onInterfaceReset={handleInterfaceReset} />
                </ProtectedRoute>
              }
            >
            ...
            {/* ============================================
                CUSTOMER ROUTES (Always defined)
                Protected by ProtectedRoute component
                ============================================ */}
            <Route 
              path="/customer" 
              element={
                <ProtectedRoute allowedRoles={['customer']}>
                  <CustomerLayout onInterfaceReset={handleInterfaceReset} />
                </ProtectedRoute>
              }
            >
            ...
            {/* ============================================
                DATABASE TEST ROUTE (Always defined)
                ============================================ */}
            <Route 
              path="/database-test" 
              element={<DatabaseTest onReset={handleInterfaceReset} />} 
            />

            {/* ============================================
                CATCH-ALL ROUTE (Always defined)
                Redirects to appropriate location
                ============================================ */}
            <Route 
              path="*" 
              element={
