...
const WarehouseConfiguration = () => {
  ...
  // Market Master Enhanced Metrics
  const calculateEnhancedMetrics = () => {
    const activeWarehouses = warehouses.filter(w => w.status === 'active').length;
    const totalItems = warehouses.reduce((sum, w) => sum + (w.total_items || 0), 0);
    const uniqueSkus = warehouses.reduce((sum, w) => sum + (w.unique_skus || 0), 0);
    
    // Calculate average utilization
    const totalUtilization = warehouses.reduce((sum, w) => {
      const used = w.total_capacity_sqft - w.available_capacity_sqft;
      return sum + ((used / w.total_capacity_sqft) * 100);
    }, 0);
    const avgUtilization = warehouses.length > 0 ? (totalUtilization / warehouses.length).toFixed(0) : 0;
    
    // Daily operations (mock for now)
    const dailyOperations = warehouses.reduce((sum, w) => sum + (w.daily_orders_fulfilled || 0), 0);
    
    return { activeWarehouses, totalItems, uniqueSkus, avgUtilization, dailyOperations };
  };

  const metrics = calculateEnhancedMetrics();

  // Get capacity level (Low/Medium/High)
  const getCapacityLevel = (warehouse) => {
    const utilization = parseFloat(getCapacityPercentage(warehouse));
    if (utilization >= 85) return { level: 'Low', color: 'text-red-600 bg-red-50' };
    if (utilization >= 60) return { level: 'Medium', color: 'text-yellow-600 bg-yellow-50' };
    return { level: 'High', color: 'text-green-600 bg-green-50' };
  };
  ...
  const handleSelectAll = (checked) => {
    if (checked) {
      setSelectedWarehouses(warehouses.map(w => w.warehouse_id));
    } else {
