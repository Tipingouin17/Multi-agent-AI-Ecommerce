...
app = FastAPI(title="Dynamic Pricing Agent API", version="1.0.0")

@app.get("/")
async def root():
    return {"agent_name": "dynamic_pricing_agent", "status": "running"}

@app.get("/health")
async def health_check():
    return {"status": "healthy", "agent_name": "dynamic_pricing_agent", "timestamp": datetime.now().isoformat()}
