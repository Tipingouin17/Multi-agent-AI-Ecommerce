...
app = FastAPI(title="Warehouse Agent API", version="1.0")

@app.get("/", summary="Root endpoint", tags=["Agent Status"])
async def root():
    return {"agent_name": "Warehouse Agent", "status": "running"}

@app.get("/health", summary="Health check endpoint", tags=["Agent Status"])
async def health_check():
