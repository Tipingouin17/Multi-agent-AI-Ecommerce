...
if (!(Test-Path "infrastructure/docker-compose.yml")) {
    Write-Host "ERROR: infrastructure/docker-compose.yml not found" -ForegroundColor Red
...
foreach ($service in $services) {
    ...
    if ($result.TcpTestSucceeded) {
        Write-Host "SUCCESS: $($service.Name) is accessible on port $($service.Port)" -ForegroundColor Green
    } else {
        if ($service.Required) {
            Write-Host "ERROR: $($service.Name) is NOT accessible on port $($service.Port)" -ForegroundColor Red
            $allHealthy = $false
        } else {
            Write-Host "WARNING: $($service.Name) is NOT accessible on port $($service.Port) (optional)" -ForegroundColor Yellow
        }
    }
}
...
if ($allHealthy) {
    Write-Host "SUCCESS: All infrastructure services are ready!" -ForegroundColor Green
    ...
    Write-Host "WARNING: Some services are not ready" -ForegroundColor Yellow
